...
tokenizer = AutoTokenizer.from_pretrained('./cross-ls-model/checkpoint-12370/', add_prefix_space=True)
model = AutoModelForTokenClassification.from_pretrained('./cross-ls-model/checkpoint-12370/', device_map="auto")

id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../cross_data/test.jsonl', 'r') as reader:
    true_labels = []
    true_predictions = []
