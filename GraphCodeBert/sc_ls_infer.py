...
tokenizer = AutoTokenizer.from_pretrained('./sc-ls-model/checkpoint-1370/', add_prefix_space=True)
model = AutoModelForTokenClassification.from_pretrained('./sc-ls-model/checkpoint-1370/', device_map="auto")

id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../sc_data/test.jsonl', 'r') as reader, open('ls_graphbert_sc.out', 'w') as res:
    true_labels = []
    true_predictions = []
