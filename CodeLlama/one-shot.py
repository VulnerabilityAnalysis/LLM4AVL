...
def analyze_code_for_vulnerabilities(code_snippet, idx):
    """
...
with open('one_codellama_13b_sc.out', 'w') as res:
    true_labels = []
    true_predictions = []
    ...
    for (idx, row) in tqdm(test_set.iterrows()):
        ...
        # Analyze the code
        analysis_results = analyze_code_for_vulnerabilities(code_snippet, idx)
