...
tokenizer = AutoTokenizer.from_pretrained('./sc-ls-model/checkpoint-1370/')
model = LlamaForTokenClassification.from_pretrained('./sc-ls-model/checkpoint-1370/').to(device)
id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../sc_data/test.jsonl', 'r') as reader, open('ls_codellama_sc.out', 'w') as res:
    true_labels = []
    true_predictions = []
    for line in tqdm(reader):
        row = json.loads(line)
        # Tokenize the entire code
        inputs = tokenizer(row['tokens'], is_split_into_words=True, return_tensors='pt', truncation=True, max_length=4096).to(device)
