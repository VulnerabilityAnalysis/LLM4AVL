...
def is_shorter_than_max_length(example):
    # Tokenize the text and check its length against the max_length_limit
    # Adjust 'text_column_name' to the name of your column containing the text to be tokenized
    text_column_name = "function"  # Adjust based on your dataset structure
...
def predict_location(code_snippet, model, tokenizer, max_input_length):
    ...
    output_sequences = model.generate(input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"], max_new_tokens=max_target_length)
...
model = AutoModelForSeq2SeqLM.from_pretrained("plbart-base-gen/checkpoint-10810").to(device)
