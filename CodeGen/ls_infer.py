...
tokenizer = AutoTokenizer.from_pretrained(f"{model_name}-ls/checkpoint-10810/")
model = CodeGenForTokenClassification.from_pretrained(f"{model_name}-ls/checkpoint-10810/").to(device)
id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../data/test.jsonl', 'r') as reader, open(f"{model_name}_temp.out", 'w') as res:
    true_labels = []
    true_predictions = []
