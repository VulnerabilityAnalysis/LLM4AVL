...
from modeling_codegen import CodeGenForTokenClassification
import json
import torch
...
tokenizer = AutoTokenizer.from_pretrained(f"{model_name}-ls-sc/checkpoint-1370/")
model = CodeGenForTokenClassification.from_pretrained(f"{model_name}-ls-sc/checkpoint-1370/").to(device)
id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../sc_data/test.jsonl', 'r') as reader, open(f"{model_name}_ls_sc.out", 'w') as res:
    true_labels = []
    true_predictions = []
