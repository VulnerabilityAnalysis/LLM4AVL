...
model = AutoModelForTokenClassification.from_pretrained('./my_awesome_ds_model/checkpoint-10810/').to(device)
id2label = {0: 'O', 1: 'B-LOC'}
with open(f'../data/test.jsonl', 'r') as reader, open('ls_codebert.out', 'w') as res:
    true_labels = []
    true_predictions = []
    for line in reader:
        row = json.loads(line)
